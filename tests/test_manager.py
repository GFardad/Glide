...
def test_cto_manager_register_and_check(tmp_path):
    manager = CTOManager(CTOManagerConfig(root=tmp_path))
...
def test_cto_manager_escalate(tmp_path):
    config = CTOManagerConfig(root=tmp_path)
    manager = CTOManager(config)
    manager.register_team("core", {"version": "1.0"})
...
def test_team_comms_send_and_status(tmp_path):
...
def test_cto_manager_start_dev_session(mock_dev_env_cls, tmp_path):
...
@patch("runtime.manager.cto_manager.DevEnvironment")
def test_cto_manager_status(mock_dev_env_cls, tmp_path):
    mock_dev = mock_dev_env_cls.return_value
...
def test_cto_manager_propose_promotion(mock_engine_cls, tmp_path):
...
def test_cto_manager_broadcast_to_all(tmp_path):
    manager = CTOManager(CTOManagerConfig(root=tmp_path))
    manager.register_team("core", {"version": "1.0"})
...
def test_cto_manager_broadcast_to_team(tmp_path):
    manager = CTOManager(CTOManagerConfig(root=tmp_path))
    manager.register_team("core", {"version": "1.0"})
...
def test_cto_manager_sync_team(tmp_path):
    manager = CTOManager(CTOManagerConfig(root=tmp_path))
    manager.register_team("core", {"version": "1.0"})
...
@patch("runtime.manager.cto_manager.DecisionEngine")
def test_cto_manager_merge_proposal(mock_engine_cls, tmp_path):
    mock_engine = mock_engine_cls.return_value
    mock_decision = type("Decision", (), {"id": "decision-1"})()
    mock_engine.decide.return_value = mock_decision
    manager = CTOManager(CTOManagerConfig(root=tmp_path))

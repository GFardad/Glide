...
function validateAgentDir(workspace, agentId) {
...
export function ensureAgentContract(workspace, agent) {
    ...
    if (!existsSync(todosPath))
        writeFileSync(todosPath, `# TODO\n\n`);
    if (!existsSync(rejectedPath))
        writeFileSync(rejectedPath, `# Rejected\n\n`);
}
export function loadAgentContract(workspace, agentId) {
...
export function appendNote(workspace, agentId, message) {
    const agentDir = validateAgentDir(workspace, agentId);
    const path = join(agentDir, "NOTES.md");
    const timestamp = new Date().toISOString();
    writeFileSync(path, `${readFileSync(path, "utf8")}\n- [${timestamp}] ${message}\n`);
}
export function markTodoDone(workspace, agentId, todo) {
    const agentDir = validateAgentDir(workspace, agentId);
...
export function recordRejection(workspace, agentId, item, reason, rejectedBy = "runtime") {
    const agentDir = validateAgentDir(workspace, agentId);
...
export function cleanupAgentWorkspace(workspace, agentId) {
    const agentDir = validateAgentDir(workspace, agentId);

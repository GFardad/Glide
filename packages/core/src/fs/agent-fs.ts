...
export function agentDir(workspace: string, agentId: string): string {
  return join(workspace, "teams", agentId, "agents", agentId);
}

export function agentFilePath(workspace: string, agentId: string, file: RequiredAgentFile): string {
  return join(agentDir(workspace, agentId), file);
}

export function validateAgentDirectory(workspace: string, agentId: string): void {
  ...
  for (const file of REQUIRED_AGENT_FILES) {
    ...
    if (file === "contract.json") {
      try {
        ...
        incomplete.push(`contract.json: ${(error as Error).message}`);
      }
    }
  }
  ...
  if (missing.length > 0 || incomplete.length > 0) {
    throw new AgentSchemaError(
      `Agent directory invalid: missing=[${missing.join(", ")}] incomplete=[${incomplete.join(", ")}]`,
      missing.length > 0 ? new Error("missing files") : undefined
    );
  }
}

export function ensureAgentFiles(workspace: string, agent: AgentContext): void {
...
export function loadAgentDirectory(workspace: string, agentId: string): AgentDirectory {
  const dir = agentDir(workspace, agentId);
  const files: AgentDirectory["files"] = {
...
export function createAgentFileContract(workspace: string, agent: AgentContext): AgentFileContract {
  ...
  return loadAgentDirectory(workspace, agent.agentId).files as unknown as AgentFileContract;
}

export function listAgentDirectories(workspace: string): string[] {
...
export function cleanupAgentDirectory(workspace: string, agentId: string): void {
  const dir = agentDir(workspace, agentId);

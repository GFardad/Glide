...
export function sanitizeWorkspacePath(candidate, allowedRoots) {
    if (!allowedRoots.length) {
        throw new CommandGuardError("CWD_OUTSIDE_WORKSPACE", "No allowed workspace roots configured; cannot sanitize cwd");
    }
    ...
    const withinAny = normalizedRoots.some((root) => normalized.endsWith(sep) ? normalized.startsWith(root) : normalized === root.slice(0, -1) || normalized.startsWith(root));
    if (!withinAny) {
        throw new CommandGuardError("CWD_OUTSIDE_WORKSPACE", `Sanitized cwd "${normalized}" is outside allowed workspace roots`);
    }
    return normalized;
}
...
export function validateArgument(arg, allowedArguments) {
    if (!allowedArguments.size) {
        return;
    }
    if (!allowedArguments.has(arg)) {
        throw new CommandGuardError("ARGUMENT_NOT_ALLOWED", `Argument "${arg}" is not in the allowlist`);
    }
}
export function runAllowedCommand(command, cwd, options = {}) {
    ...
    if (!allowedCommands.has(commandName)) {
        throw new CommandGuardError("COMMAND_NOT_ALLOWED", `Command "${commandName}" is not in the allowlist`);
    }
    ...
    if (result.status !== 0) {
        const message = result.stderr?.trim() || `Command exited with code ${result.status}`;

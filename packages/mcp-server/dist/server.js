...
import { listPlugins, resolveRegistryPath, } from "@glide/core";
...
function resolveRoleFromArgs(args) {
    const candidates = [
        args.subject_role,
        args.role,
        args.agent_role,
...
function isToolAllowedForRole(tool, role) {
    if (!role)
        return true;
...
function authorizeToolCall(tool, args) {
    ...
    const resource = args.project_path ??
        args.campaign_dir ??
        args.workspace ??
...
async function loadDynamicPlugins(root = DEFAULT_PROJECT_ROOT) {
...
export function createGlideServer(pluginRoot = DEFAULT_PROJECT_ROOT) {
    let allTools = [...tools];
    const server = new Server({ name: "glide", version: "0.1.0" }, { capabilities: { tools: { listChanged: false } } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        const parsed = parseToolArguments(tool, arguments_);

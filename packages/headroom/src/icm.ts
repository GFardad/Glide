...
function normalizeStage(stage: number): string {
...
export function buildIcmCampaign(input: IcmCampaignInput): IcmCampaignResult {
  ...
  const stageSummaries: { stage: string; dir: string }[] = [];
  ...
  writeTextFile(
    join(root, "contracts", "campaign.json"),
    JSON.stringify(
      {
        id,
...
export function runWalkTest(root: string): IcmCampaignResult["walkTest"] {
  const required = [
    "GOAL.md",
    "NON_GOALS.md",
    "ASSUMPTIONS.md",
  ...
  const readableStateFiles: string[] = [];

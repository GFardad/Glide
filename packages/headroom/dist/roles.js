...
function isBlocking(objective) {
    const normalized = objective.toLowerCase();
    return BLOCKING.some((token) => normalized.includes(token));
}
function keywordSignals(objective, role) {
    ...
    return unique.filter((token) => hintTerms.some((hint) => token.includes(hint) || hint.includes(token)));
}
function buildAssessment(role, objective) {
    if (isBlocking(objective)) {
...
function buildRisks(role, objective) {
    if (isBlocking(objective)) {
...
function buildImprovements(role, objective) {
    const improvements = {
        Architect: [
...
function buildTodos(role, objective) {
    if (isBlocking(objective)) {
...
export async function runRoleAnalysis(objective, roles, campaignDir) {
    const analysis = {};
